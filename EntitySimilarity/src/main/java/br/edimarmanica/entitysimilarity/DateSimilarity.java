 ...
 * @author edimar
 */
public class DateSimilarity extends TypeAwareSimilarity {
    ...
    public double similaritySpecific(String valueR1, String valueR2) {
        ...
        try {
            List<String> vR1formats = getAllFormats(valueR1);
            List<String> vR2formats = getAllFormats(valueR2);

            for (String fr : vR1formats) {
                for (String fs : vR2formats) {
                    if (fr.equals(fs)) {
                        return 1;
                    }
                }
